...
(() => {
  ...
  const openEditor = (type, id) => {
    ...
    form.onsubmit = async (event) => {
      ...
      const initialLabel = button.textContent; button.disabled = true; button.textContent = "جارٍ الحفظ…"; byId("admin-edit-error").textContent = "";
      try {
        if (type === "subject") {
          const saved = await rpc("admin_update_ninth_subject", { p_id: record.id, p_name: safeText(values.get("name")), p_slug: record.slug, p_description: safeText(values.get("description")), p_icon_key: values.get("icon"), p_color_from: values.get("from"), p_color_to: values.get("to"), p_sort_order: Number(values.get("sort") || 0), p_is_active: values.has("active") });
          if (!(saved?.id || (Array.isArray(saved) && saved[0]?.id))) throw new Error("لم تؤكد قاعدة البيانات حفظ التعديل.");
        } else {
      ...
      finally { button.disabled = false; button.textContent = initialLabel; }
    };
    dialog.showModal();
  };
