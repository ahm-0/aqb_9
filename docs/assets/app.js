...
function openViewer(raw, title) { const url = directDriveUrl(raw); const openInsideApp = true; if (window.Android?.openNativePdfViewer) return window.Android.openNativePdfViewer(url, title, openInsideApp); if (window.AppBridge?.openNativePdfViewer) return window.AppBridge.openNativePdfViewer(url, title, openInsideApp); if (window.AppBridge?.openPdfViewer) return window.AppBridge.openPdfViewer(url, title); if (window.AppBridge?.openExternalUrl) return window.AppBridge.openExternalUrl(url); window.open(url, "_blank", "noopener"); }
